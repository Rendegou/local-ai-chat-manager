...
| `access` | 含义 | 出现在「添加来源」 | 可配置 |
| --- | --- | --- | --- |
| `native` | 有内置适配器 | 未连接时 | 目录 |
| `import` | 只支持手动导入标准包 | 是 | 无 |
| `pending` | 认识这个工具、还没写适配器 | 是，标「待适配」 | 无 |
| （`isCustom`） | 用户填了字段映射自己接上的来源 | 已连接，不在候选里 | 目录 + 字段映射 |

- **`pending` 必须能被看见**。把「产品不支持」变成「还没做」，用户才不会以为永远不会有；
  每个 pending 行都给一个「用自定义来源接入」的入口——把「我们还没做」直接接上「那我自己接」。
  `description` 里的路径一律标注「未在本机验证」，不写成看起来权威的默认路径。
- **`isCustom` 与 `access` 是两回事**，界面判断「配置」打开哪套表单要用前者：
  一个原本判为 `pending` 的工具被字段映射接上之后，它的配置方式已经是映射，而不是「待适配」。
  后端在 `source_catalog()` 里把这类来源的 `access` 改报为 `native`，保持一致。
- 自定义来源的**显示名由用户给**，优先级高于 Catalog 与数据库里的值。
- 自定义来源**不能占用非 `pending` 的内置标识**：那会让两个适配器同 id 抢同一批会话。
- 字段映射填错**不能静默成功**：一条消息都读不出时后端返回明确错误，界面上还有「试解析」
  能在保存前就看出这个目录能读出什么。
...
| 添加来源抽屉 | 未连接的 Catalog 来源（含 `pending` 待适配清单） |
...
- **主题与语言不属于「草稿 + 保存」**：它们改完立刻生效并立刻落盘，且标题栏「更多操作」里有内联
  快捷入口。否则会出现「界面已经变了，却被告知有未保存修改、离开时点放弃还会翻回去」的矛盾——
  即时生效的偏好不该受保存语义约束。
